...
\noindent\hspace{-1cm} Repeat the following until convergence:
    ...
    \item Compute the ELBO objective from Equation (\ref{eq:elbo_bayes2}). %, where the last term may be approximated using any of the methods from Section $\ref{sec:approx}$.
...
This is equivalent to the refined ELBO in (\ref{eq:elbo_bayes2}). Since we are perturbing the latent variables in the steepest direction, we show easily that, for a moderate $\eta$, the previous bound is tighter than
